...
> **Bounded context map (BE):** read `../AGENTS.md` at monorepo root.
...
modules/                  Business logic — nested sub-modules per bounded context
  ...
  ...                     See bounded-context map in ../AGENTS.md
  {context}/
    {sub-module}/
...
### 1. Declare endpoints with `apiPath`
...
export const FOO_ENDPOINTS = {
  list: (orgId: string, params?: { limit?: number; offset?: number }) => {
    ...
    const q = p.toString()
    return apiPath(`/organizations/${orgId}/foos`) + (q ? `?${q}` : '')
  },
  get: (orgId: string, fooId: string) => apiPath(`/organizations/${orgId}/foos/${fooId}`),
  create: (orgId: string) => apiPath(`/organizations/${orgId}/foos`),
  patch: (orgId: string, fooId: string) => apiPath(`/organizations/${orgId}/foos/${fooId}`),
  delete: (orgId: string, fooId: string) => apiPath(`/organizations/${orgId}/foos/${fooId}`),
} as const
```

- All URLs must go through `apiPath()` — builds `/api/...` (unversioned, matches BE).
...
| [`../AGENTS.md`](../AGENTS.md)                     | Monorepo bounded context map (BE alignment)                 |
