...
import { Typography, Button, ContentLoader, Select, Textarea } from '@/shared/ui'
import {
...
import * as playgroundService from '@/services/ai-prompt-playground.service'
import * as aiAgentService from '@/services/ai-agent-control.service'
import { toast } from 'sonner'
import { getProblemToastMessage } from '@/shared/lib/errorHandling'
import { AIRunFeedbackControls } from './ai-run-feedback-controls'
import type {
  PromptActualTestResult,
  PromptDryRunResult,
  PromptPlaygroundContext,
} from '@/types/ai-prompt-playground'
import type { AIAgentVersionDetail, AIRunMode } from '@/types/ai-agent-control'
...
export function AIAgentPlaygroundPanel({ agentId, orgId }: AIAgentPlaygroundPanelProps) {
  ...
  const loadContext = useCallback(async () => {
    setLoading(true)
    try {
      const data = await playgroundService.getPlaygroundContext(agentId, orgId)
      setContext(data)
  ...
  const handleDryRun = async () => {
    ...
    try {
      const result = await playgroundService.dryRunPromptTest(agentId, payload)
  ...
  const handleRunTest = async () => {
    ...
    try {
      const result = await playgroundService.runPromptTest(agentId, payload)
  ...
  return (
    <div className="space-y-6">
      <Typography variant="small" tone="muted">
        Dry run validates prompt rendering without calling the AI provider. Run test will call the AI
        provider and may consume budget. Testing a draft version does not publish it. Output is not
        saved to production data.
      </Typography>
          ...
          {versionDetail ? (
            <div className="rounded-md border border-neutral-100 bg-neutral-50 p-3 text-sm">
              ...
              <Typography variant="xs" tone="muted">
                ...
                {versionDetail.userPromptTemplate ? `${versionDetail.userPromptTemplate.length} chars` : 'default'}
              </Typography>
            </div>
          ) : null}
        </div>
      ...
      {dryRunResult ? (
        <div className="space-y-3 rounded-lg border border-neutral-200 bg-neutral-50 p-4 text-sm">
          ...
          <p>
            Variables: {dryRunResult.renderedPromptSummary.variableKeys.join(', ') || 'none'}
          </p>
          <p>
          ...
          {!testResult.success ? (
            ...
            <>
              ...
              {testResult.estimatedCost != null ? (
                <p>
                  Cost: {formatEstimatedCost(testResult.estimatedCost, testResult.currency ?? 'USD')}
                </p>
              ) : null}
              {testResult.latencyMs != null ? <p>Latency: {testResult.latencyMs}ms</p> : null}
              {testResult.outputValidation ? (
                <p className={testResult.outputValidation.valid ? 'text-green-700' : 'text-amber-700'}>
                  Output validation:{' '}
