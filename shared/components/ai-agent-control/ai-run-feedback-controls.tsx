...
import type { AIFeedbackCategory } from '@/types/ai-run-feedback'
import { FEEDBACK_CATEGORIES } from '@/types/ai-run-feedback'
...
export function AIRunFeedbackControls({ orgId, runId, compact = false }: AIRunFeedbackControlsProps) {
  ...
  const submitRating = async (rating: 'positive' | 'negative', withDialog = false) => {
    ...
    setSubmitting(true)
    try {
      await feedbackService.submitRunFeedback(orgId, runId, {
        rating,
