    ...
    >
      <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-1">
        ...
        <div className="rounded-lg border border-neutral-200 bg-neutral-50/80 p-4">
          <Typography as="p" size="sm" weight="medium" className="text-neutral-700 mb-3">
            Recommended flow
          </Typography>
          <div className="flex flex-wrap items-center gap-2">
            {FLOW_ITEMS.map(({ icon: Icon, label }, i) => (
              <span key={i} className="flex items-center gap-2">
                ...
                <Typography as="span" variant="small" weight="medium" className="text-neutral-700 hidden sm:inline">
                  {label}
                </Typography>
                {i < FLOW_ITEMS.length - 1 && (
                  <ArrowRight size={14} className="text-neutral-400 shrink-0" aria-hidden />
                )}
              </span>
            ))}
          </div>
        ...
        {SECTIONS.map((section, i) => (
          ...
          >
            <div className="flex gap-3 mb-3">
              <span
                className={cn(
                  'inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 text-primary'
                )}
              >
            ...
            {section.image && (
              <div className="mb-3 rounded-lg border border-neutral-200 overflow-hidden bg-neutral-50">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={section.image}
                  alt=""
                  className="w-full max-h-48 object-contain object-left-top"
                />
              </div>
            )}
            <ul className="space-y-2 pl-1">
              {section.steps.map((step, j) => (
                <li key={j} className="flex gap-2 text-sm text-neutral-700">
                  <ChevronRight size={14} className="shrink-0 mt-0.5 text-primary/70" aria-hidden />
