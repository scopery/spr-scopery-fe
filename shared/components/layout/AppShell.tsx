...
export function AppShell({ orgId, children }: AppShellProps) {
  ...
  const projectsActive =
    pathname === ROUTES.org.projects(orgId) || pathname?.startsWith(ROUTES.org.projects(orgId) + '/')
  const documentHubActive =
    pathname === ROUTES.org.documentHub(orgId) || pathname?.startsWith(ROUTES.org.documentHub(orgId) + '/')
  ...
  useEffect(() => {
    ...
    let cancelled = false
    Promise.all([
      accessService.getEffectivePermissions(orgId).catch(() => null),
  ...
  return (
    <Box as="div" className="min-h-screen flex flex-col bg-neutral-50">
      <Box
        as="header"
      ...
      >
        <Box as="div" className="flex min-w-0 items-center">
          <DesignLink
            ...
            className="flex shrink-0 items-center border-r border-neutral-300 px-6 rounded-none"
          ...
          {currentOrg && (
            <Box as="div" className="relative">
              <Button
                ...
                aria-haspopup="menu"
                icon={<Avatar size="xs" fallback={orgInitial(currentOrg.name)} alt={currentOrg.name} />}
                iconRight={<ChevronsUpDown size={14} className="text-neutral-400" />}
              >
                <Typography as="span" weight="medium" className="max-w-[160px] truncate sm:max-w-[220px]">
                  {currentOrg.name}
                </Typography>
              </Button>
                  ...
                  >
                    <Box as="div" padding="md" className="border-b border-neutral-100">
                      <Stack direction="horizontal" spacing="sm" align="start">
                        <Avatar size="sm" fallback={orgInitial(currentOrg.name)} alt={currentOrg.name} />
                        <Stack direction="vertical" spacing="none" className="min-w-0 flex-1">
                          <Stack direction="horizontal" spacing="xs" align="center" wrap>
                            ...
                            <Badge variant="soft" tone="primary" size="sm" className="uppercase tracking-wide">
                              {currentOrg.my_role}
                            </Badge>
                          </Stack>
                    ...
                    <DesignLink
                      ...
                      role="menuitem"
                      className="flex items-center gap-2 px-4 py-2 text-sm hover:bg-neutral-50 rounded-none"
                      onClick={() => setWorkspaceMenuOpen(false)}
                    >
                      ...
                      <>
                        ...
                        {otherOrgs.map((org) => (
                          <Button
                            ...
                            icon={<Avatar size="xs" fallback={orgInitial(org.name)} alt={org.name} />}
                          >
          ...
          {currentOrg && (
            <>
              <DesignLink
                ...
                className={cn(
                  'hidden items-center gap-2 border-l border-neutral-300 px-4 text-sm rounded-none transition-colors sm:flex',
                  projectsActive ? 'text-neutral-900 bg-neutral-100' : 'text-neutral-600 hover:bg-neutral-100'
                )}
              >
              ...
              <DesignLink
                ...
                className={cn(
                  'hidden items-center gap-2 border-l border-neutral-300 px-4 text-sm rounded-none transition-colors sm:flex',
                  documentHubActive ? 'text-neutral-900 bg-neutral-100' : 'text-neutral-600 hover:bg-neutral-100'
                )}
              >
        ...
        <Box as="div" className="flex items-center gap-2 px-3">
          ...
          >
            Search
            <Typography as="span" variant="small" tone="muted" className="ml-1 border border-neutral-200 px-1.5 py-0.5 text-[10px]">
              ⌘K
            </Typography>
          </Button>
