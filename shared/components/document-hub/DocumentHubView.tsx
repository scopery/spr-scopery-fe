...
import { Button, Input, Select, Typography, ContentLoader, Badge, Modal, Checkbox } from '@/shared/ui'
import { ROUTES } from '@/constants/routes'
import * as documentHubService from '@/services/document-hub.service'
import * as documentsService from '@/services/documents.service'
import * as projectService from '@/services/project.service'
import { AIGeneratedBadge, originLabel } from '@/shared/components/ai-document-intelligence/AIGeneratedBadge'
...
import { DocumentHubExportDialog, type DocumentHubExportOptions, type DocumentHubSelectionMode } from './DocumentHubExportDialog'
import * as documentExportService from '@/services/document-export.service'
import { DOCUMENT_TYPE_OPTIONS, DOCUMENT_WORKFLOW_STATUS_OPTIONS, type DocumentWorkflowStatus } from '@/types/document'
...
export function DocumentHubView({
  ...
  const [projects, setProjects] = useState<projectService.ProjectListItem[]>([])
  const [items, setItems] = useState<documentHubService.DocumentHubItem[]>([])
  ...
  useEffect(() => {
    projectService.listProjects(orgId, { limit: 100 }).then((res) => setProjects(res.items)).catch(() => {})
  }, [orgId])

  const load = useCallback(async () => {
    setLoading(true)
    try {
      const res = await documentHubService.listDocumentHubDocuments(orgId, {
        project_id: projectId || undefined,
        search: debouncedSearch || undefined,
        document_type: (documentType || undefined) as import('@/types/document').DocumentType | undefined,
  ...
  }, [orgId, projectId, debouncedSearch, documentType, originType, aiOnly, lifecycleStatus, workflowStatus])
  ...
  }, [projectId, debouncedSearch, documentType, originType, aiOnly, lifecycleStatus, workflowStatus])
  ...
  const runHubExport = async (options: DocumentHubExportOptions, previewUsed: boolean) => {
    ...
    try {
      await documentExportService.exportDocumentHub(orgId, {
        mode,
      ...
      toast.success(
        mode === 'filtered'
          ? 'Filtered documents exported'
          : 'Selected documents exported'
      )
      setExportOpen(false)
      clearSelection()
    } catch (err) {
  ...
  const openDeliverableWithProject = (resolvedProjectId: string, docIds: string[], titles: string[]) => {
  ...
  const openDeliverableFlow = async () => {
    if (selectionMode === 'filtered_all') {
      ...
      try {
        const resolved = await deliverablesService.resolveHubDeliverableSelection(
          orgId,
          currentFilters()
        )
    ...
    if (docIds.length > 0) {
      setDeliverableLoading(true)
      try {
        const context = await deliverablesService.getHubDeliverableContext(orgId, docIds)
  ...
  const handleRestoreDocument = async (doc: documentHubService.DocumentHubItem) => {
    if (!doc.project_id) {
    ...
    try {
      await documentsService.restoreDocument(orgId, doc.id, doc.project_id)
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7">
        <div className="relative lg:col-span-1">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none" size={16} />
          <Input
        ...
        <Select
          ...
          onValueChange={(v: string) => setWorkflowStatus(v === '__all__' ? '' : (v as DocumentWorkflowStatus))}
          options={[
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          ...
          {items.map((doc) => {
            ...
            const isSelected =
              selectionMode === 'filtered_all' || selectedIds.has(doc.id)
            const cardContent = (
              <>
            ...
            if (isArchived) {
              return (
                <div
                  key={doc.id}
                  className="block border border-neutral-200 bg-neutral-50 p-4"
                >
                  {cardContent}
                </div>
              )
            ...
            return (
              <Link
                ...
                className="block border border-neutral-200 bg-white p-4 hover:border-neutral-300 transition-colors"
              >
                {cardContent}
              </Link>
            )
      ...
      <Modal open={pickProjectOpen} onClose={() => {
        setPickProjectOpen(false)
        setPickProjectForDeliverable(false)
      }} title="Choose project" size="sm">
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button
              variant="primary"
              onClick={pickProjectForDeliverable ? confirmProjectForDeliverable : confirmProjectForCreate}
            >
              Continue
            </Button>
          </div>
