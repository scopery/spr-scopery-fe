...
export function InlineComboboxInput({
  ...
  const { inputProps, inputRef, showTrigger, trigger, setSearch, itemIds, activeId, setActiveId, removeInput, selectActiveItem } =
    ctx
...
export function InlineComboboxItem({
  ...
  const visible =
    ctx.filter === false ||
    ctx.filter({ value, label, keywords, group }, ctx.search)
