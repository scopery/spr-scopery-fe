...
import type { CollaborationPermissions, DocumentSuggestion } from '@/types/collaboration'
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="flex-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Badge variant="soft" tone="neutral" size="sm">{suggestion.status}</Badge>
            <Typography variant="small" tone="muted">
              {new Date(suggestion.created_at).toLocaleString()}
            </Typography>
          </div>
          {suggestion.title && (
            <Typography weight="medium" className="mt-1">{suggestion.title}</Typography>
          )}
...
export function DocumentSuggestionsPanel({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await collaborationService.listSuggestions(orgId, documentId, {
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true)
    try {
      await collaborationService.createSuggestion(orgId, documentId, {
    ...
    <div className="space-y-4">
      ...
      {permissions.canCreateSuggestion && (
        <div className="space-y-2 border border-neutral-200 p-3">
          <Input label="Title (optional)" value={title} onChange={(e) => setTitle(e.target.value)} fullWidth />
          <Textarea
            label="Suggestion"
            value={description}
            ...
            fullWidth
          />
          <MentionUserPicker orgId={orgId} projectId={projectId} value={mentions} onChange={setMentions} />
          <Button variant="primary" size="sm" loading={submitting} onClick={() => void handleCreate()}>
            Submit suggestion
          </Button>
        </div>
      )}
        ...
        <div className="space-y-3">
          {suggestions.map((s) => (
            <SuggestionCard
              ...
              onAccept={async (id) => {
                await collaborationService.acceptSuggestion(orgId, documentId, id, projectId)
                await load()
              }}
              onReject={async (id) => {
                await collaborationService.rejectSuggestion(orgId, documentId, id, projectId)
                await load()
              }}
            />
          ))}
