...
import type { CollaborationPermissions, DocumentComment } from '@/types/collaboration'
...
export function DocumentCommentsPanel({
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await collaborationService.listComments(orgId, documentId, {
  ...
  const handleCreate = async () => {
    ...
    setSubmitting(true)
    try {
      await collaborationService.createComment(orgId, documentId, {
        body: body.trim(),
        project_id: projectId,
        ...
        <div className="space-y-3">
          {comments.map((comment) => (
            <DocumentCommentThread
              ...
              onReply={handleReply}
              onResolve={async (id) => {
                await collaborationService.resolveComment(orgId, documentId, id, projectId)
                await load()
              }}
              onReopen={async (id) => {
                await collaborationService.reopenComment(orgId, documentId, id, projectId)
                await load()
              }}
            />
          ))}
