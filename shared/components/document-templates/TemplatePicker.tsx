...
export function TemplatePicker({ orgId, selectedId, onSelect }: TemplatePickerProps) {
  ...
  const load = useCallback(async () => {
    ...
    setError(null)
    try {
      const result = await documentTemplatesService.listAvailableTemplates(orgId, { limit: 50 })
  ...
  if (templates.length === 0) {
    return (
      <Typography variant="small" tone="muted" className="py-4">
        No templates available. Run the document template seed on the backend, or create a personal template.
      </Typography>
    )
  }
