...
import { DOCUMENT_TYPE_OPTIONS, type DocumentType } from '@/types/document'
import type { DocumentTemplate, TemplateScope, TemplateVariableDefinition } from '@/types/document-template'
import { TemplateScopeBadge } from './TemplateScopeBadge'
import { TemplateStatusBadge } from './TemplateStatusBadge'
import { PlateEditorBody, type PlateEditorHandle } from '@/shared/components/editor/PlateEditor'
import { contentToPlateValue, plateValueToContent } from '@/shared/components/editor/content-adapter'
...
import { buildVariableSlashGroups } from './variables/template-variable-slash-items'
...
export function TemplateEditor({
  ...
  const canEdit = mode === 'create' || (template ? canEditTemplate(template, userId, isAdmin) : false)
  ...
  const [plateValue, setPlateValue] = useState<Value>(() =>
    contentToPlateValue(template?.content ?? { format: 'plate', value: [{ type: 'p', children: [{ text: '' }] }] })
  )
  ...
  useEffect(() => {
    void documentTemplatesService.listTemplateVariables(orgId).then(setVariableDefinitions).catch(() => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (mode === 'create') {
        saved = await documentTemplatesService.createTemplate(orgId, {
          title: title.trim(),
        ...
        saved = await documentTemplatesService.updateTemplate(orgId, template.id, {
          title: title.trim(),
          description: description.trim() || null,
          category: category || null,
          document_type: documentType,
      ...
      setSaveStatus('error')
      const msg =
        err instanceof ApiError ? err.problem.detail : err instanceof Error ? err.message : 'Failed to save'
      toast.error(msg)
    } finally {
      setLoading(false)
    }
  }, [orgId, mode, template, title, description, scope, category, documentType, plateValue, status, onSaved])
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        {showPublishActions && (
          <Button variant="outline" size="sm" loading={loading} onClick={() => void handlePublishToggle()}>
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_320px]">
          ...
          {canEdit && variableDefinitions.length > 0 && (
            <TemplateVariablePanel
              ...
              className="border-t lg:border-t-0 lg:border-l border-neutral-200"
            />
          )}
        </div>
      </div>
