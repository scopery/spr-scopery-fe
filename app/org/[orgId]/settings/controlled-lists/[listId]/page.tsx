...
import { Box, Button, Input, Typography, Stack, Textarea, Badge, Switch, Spinner } from '@/shared/ui'
import { ROUTES } from '@/constants/routes'
import { useControlledListDetail, useControlledValues } from '@/hooks'
import type { ControlledList } from '@/types/controlled-lists'
...
export default function ControlledListDetailPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const sort =
        sortOrder.trim() !== '' ? Number.parseInt(sortOrder, 10) : undefined
    ...
    <main className="min-h-screen bg-neutral-50 p-8">
      <Box padding="xl" background="white" shadow="sm" className="border-b border-neutral-200">
        <Stack direction="vertical" spacing="md">
          <Stack direction="horizontal" justify="between" align="center" className="flex-wrap gap-md">
          ...
          {(listError || valuesError) && (
            <Box
              padding="md"
              className="border border-error/20 rounded-lg bg-error/5"
              role="alert"
            >
          ...
          {!listError && (
            <Box
              padding="md"
              className="border border-warning/20 rounded-lg bg-warning/5"
              role="status"
            >
      ...
      <Box padding="xl" className="max-w-5xl mx-auto space-y-xl">
        <Box
          padding="lg"
          background="white"
          radius="lg"
        ...
        >
          <Stack direction="vertical" spacing="md">
            ...
            {valuesLoading && values.length === 0 ? (
              <Box
                padding="lg"
                className="text-center border border-neutral-200 rounded-lg bg-neutral-50"
              >
                <Spinner size="sm" />
              </Box>
            ) : values.length === 0 ? (
              <Box
                padding="lg"
                className="text-center border border-dashed border-neutral-200 rounded-lg bg-neutral-50"
              >
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-neutral-200 bg-neutral-50">
                      ...
                      <th className="p-md text-sm font-semibold text-neutral-700">
                        Description
                      </th>
