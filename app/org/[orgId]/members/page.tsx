...
import { Typography, Badge, Button, ContentLoader, Select } from '@/shared/ui'
...
import { CreateInviteModal } from './_components/CreateInviteModal'
...
export default function OrgMembersPage() {
  ...
  const { members, loading: membersLoading, patchMember, removeMember, leave } = useOrgMembers(orgId)
  ...
  const [confirmRevoke, setConfirmRevoke] = useState<{ inviteId: string; email: string } | null>(null)
  ...
  const handleRevokeInvite = async () => {
    ...
    setActionLoading(true)
    try {
      await orgInviteService.revokeInvite(orgId, confirmRevoke.inviteId)
      ...
      {tab === 'members' && (
        <>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-500">
                  ...
                  {isOwner && <th className="px-4 py-3 text-left font-normal text-neutral-700 w-48">Actions</th>}
                </tr>
              </thead>
              <tbody>
                {members.map((m) => (
                  <tr key={m.user_id} className="border-b border-neutral-200 last:border-0">
                    ...
                    <td className="px-4 py-3">
                      {isOwner && m.user_id !== currentUserId ? (
                        <Select
                          ...
                          value={m.role}
                          onValueChange={(v: string) => handleChangeRole(m.user_id, v as 'owner' | 'member' | 'partner')}
                          size="sm"
                    ...
                    <td className="px-4 py-3">
                      <Badge variant="solid" tone={m.status === 'active' ? 'success' : 'neutral'} size="sm">
                        {m.status}
                      </Badge>
                    </td>
                    {isOwner && (
                      <td className="px-4 py-3">
                        {m.user_id !== currentUserId && m.status === 'active' && (
                          <Button
                            ...
                            size="sm"
                            tone="error"
                            onClick={() => setConfirmRemove({ userId: m.user_id, displayName: m.display_name })}
                            disabled={isPartner}
                          >
                            Remove
                          </Button>
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" onClick={() => setConfirmLeave(true)} disabled={isPartner}>
              <LogOut size={12} />
              Leave organization
            </Button>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-neutral-500">
                  ...
                  <th className="px-4 py-3 text-left font-normal text-neutral-700 w-24">Actions</th>
                </tr>
              </thead>
              <tbody>
                {invites.length === 0 ? (
