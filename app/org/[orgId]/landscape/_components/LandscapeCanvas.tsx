...
import * as landscapeService from '@/services/landscape.service'
import type { OrgNode, OrgNodeLink, NodeLinkType } from '@/services/landscape.service'
...
function OrgNodeCard({ data, selected }: NodeProps<Node<OrgNodeData>>) {
  return (
    ...
    >
      ...
      <div className="text-sm font-mono text-neutral-600">{data.code}</div>
      <div className="text-sm font-medium text-neutral-900 truncate">{data.name}</div>
      <div className="text-xs text-neutral-500 capitalize">{data.node_type}</div>
...
export function LandscapeCanvas({
  ...
  const flushPositions = useCallback(async () => {
    ...
    try {
      await landscapeService.batchUpdateNodePositions(orgId, positions)
    } catch {
      // Fallback: PATCH each node individually (e.g. if batch endpoint not available)
      await Promise.all(
        positions.map((p) =>
          landscapeService.patchOrgNode(orgId, p.node_id, {
  ...
  const onConnect: OnConnect = useCallback(
    async (connection: Connection) => {
      ...
      try {
        await landscapeService.createNodeLink(orgId, {
