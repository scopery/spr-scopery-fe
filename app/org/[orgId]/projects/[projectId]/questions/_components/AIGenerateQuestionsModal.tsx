...
import { useQuestionsGenerate, useQuestionsCommit } from '@/features/ai'
import type { ProjectDetail } from '@/services/project.service'
import type { GeneratedQuestionItem, QuestionsCommitEdit, QuestionsCommitPatch, QuestionsGenerateBody } from '@/features/ai'
...
export function AIGenerateQuestionsModal({
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      res.items.forEach((i) => { acc[i.temp_id] = true })
      setAccepted(acc)
      setEdits({})
      setStep('review')
    } catch (err) {
  ...
  }, [orgId, projectId, instruction, engine, maxItems, baselineSessionId, defaultBaseline, generate])
  ...
  const setEdit = (tempId: string, field: keyof GeneratedQuestionItem, value: string | boolean | string[]) => {
    setEdits((prev) => ({
      ...prev,
    ...
    >
      {step === 'config' ? (
        <div className="space-y-4">
          ...
          />
          <div>
            <label className="block text-sm text-neutral-700 mb-1">Engine</label>
            <Select
              options={[
              ...
              value={engine}
              onValueChange={(v: string) => setEngine(v === 'legacy' ? 'legacy' : 'agentkit_v2_file')}
              className="w-full"
            />
            <Typography variant="small" tone="muted" className="mt-1">
              {engine === 'agentkit_v2_file' ? 'V2 requires a session (base_session_id).' : 'Legacy requires at least one session or revision.'}
            </Typography>
          </div>
          {baselineOptions.length > 0 ? (
            <div>
              <label className="block text-sm text-neutral-700 mb-1">Session <span className="text-error-600">*</span></label>
              <Select
            ...
            <Typography variant="small" tone="muted">Create a session with answers first. V2 requires base_session_id.</Typography>
          )}
          <div>
            <Input
              ...
              return (
                <div key={item.temp_id} className=" space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) => setEdit(item.temp_id, 'tags', e.target.value.split(',').map((s) => s.trim()).filter(Boolean))}
                    fullWidth
                  />
                </div>
              )
