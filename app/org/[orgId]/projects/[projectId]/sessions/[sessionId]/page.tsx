...
import { Typography, Button, Badge, Input, Textarea, Switch, Select, ContentLoader, Divider } from '@/shared/ui'
...
import * as aiClarityService from '@/services/aiClarity'
...
import type { ClarityAssessment } from '@/types/aiClarity'
import { ProjectStepIndicator } from '../../_components/ProjectStepIndicator'
import { AIImproveModal } from './_components/AIImproveModal'
import { AIImproveAllModal } from './_components/AIImproveAllModal'
import { ClarityBadge } from './_components/ClarityBadge'
import { ClarityPanel } from './_components/ClarityPanel'
import { ClarityDetailsModal } from './_components/ClarityDetailsModal'
...
import { AnswerEvidenceStrip } from '@/shared/components/documents/AnswerEvidenceStrip'
...
function answerValuesEqual(local: unknown, server: unknown): boolean {
  ...
  const toStr = (v: unknown): string => {
    ...
    if (v !== null && typeof v === 'object' && 'text' in v) return String((v as { text?: unknown }).text ?? '')
    ...
    switch (q.q_type) {
      ...
      case 'number':
        return (
          <Input
            type="number"
            value={(value as { number?: number })?.number ?? (typeof value === 'number' ? value : '')}
    ...
    <div className="bg-white">
      <div className="flex items-start justify-between gap-4 mb-3">
        <div className="flex items-center gap-2 min-w-0">
          {q.required && (
            <Badge variant="solid" tone="warning" size="sm" className="shrink-0">Required</Badge>
          )}
          <Typography weight="semibold">{q.prompt}</Typography>
        </div>
        <div className="flex items-center gap-1 shrink-0 flex-wrap justify-end">
          {FEATURES.aiClarityAssessment && showAssessClarityButton && onAssessClarity != null && !clarityFeatureDisabled && (
            <Button
              variant="ghost"
              size="sm"
...
export default function SessionDetailPage() {
  ...
  const [rightPanelTab, setRightPanelTab] = useState<'progress' | 'outline' | 'clarity' | 'evidence'>('progress')
  const [claritySummary, setClaritySummary] = useState<import('@/types/aiClarity').ClaritySummary | null>(null)
  ...
  const [assessmentsByOrder, setAssessmentsByOrder] = useState<Record<number, import('@/types/aiClarity').ClarityAssessment>>({})
  ...
  const saveAnswers = useCallback(
    async (newAnswers: Array<{ question_id: string; answer_status: AnswerStatus; value: unknown; skip_reason?: string | null }>) => {
      ...
      try {
        ...
        const serializeValueForApi = (a: (typeof newAnswers)[0]): unknown => {
          ...
          if (question && (question.q_type === 'text' || question.q_type === 'textarea')) {
            ...
            if (a.value !== null && typeof a.value === 'object' && 'text' in a.value) return (a.value as { text?: string }).text ?? ''
            return ''
          }
          return a.value
        }
        const res = await sessionService.putAnswers(orgId, projectId, sessionId, {
        ...
        if (thisSaveId !== saveIdRef.current) return
        if (err instanceof ApiError && (getProblemCode(err) === 'SESSION_LOCKED' || err.problem.type?.includes('session-locked'))) {
  ...
  const handleAssessClarity = useCallback(
    async (order: number, q: ProjectQuestion, answer: AnswerItem | undefined) => {
      ...
      const answerText = toAnswerText(q.q_type, answer?.value, q.answer_schema as Record<string, unknown>)
      setLoadingByOrder((prev) => ({ ...prev, [order]: true }))
      try {
        const res = await aiClarityService.assessOne(orgId, projectId, sessionId, {
  ...
  const handleExportExcel = useCallback(() => {
    ...
    const rows = orderedQuestions.map((q) => {
      ...
      const answerText = toAnswerText(q.q_type, ans?.value, q.answer_schema as Record<string, unknown>)
      return [
        q.section || 'general',
        order ?? '',
        q.prompt,
  ...
  const handleBulkAssess = useCallback(() => {
    if (!session?.questions || !orgId || !projectId || !sessionId || featureDisabled || !canAssessClarity) return
      ...
      for (const q of toAssess) {
        ...
        try {
          const res = await aiClarityService.assessOne(orgId, projectId, sessionId, {
            ...
            section: q.section || 'general',
            question_text: q.prompt,
            answer_text: toAnswerText(q.q_type, ans?.value, q.answer_schema as Record<string, unknown>),
            q_type: q.q_type || null,
            required: !!q.required,
          })
          setAssessmentsByOrder((prev) => ({ ...prev, [res.question_order]: res.assessment }))
    ...
    run()
  }, [session?.questions, orgId, projectId, sessionId, featureDisabled, canAssessClarity, orderedQuestions, questionOrderMap, answers, assessmentsByOrder, loadClaritySummary])
  ...
  const handleLock = async () => {
    ...
    setLockLoading(true)
    try {
      await sessionService.lockSession(orgId, projectId, sessionId)
  ...
  const handleReopen = async () => {
    ...
    try {
      await sessionService.reopenSession(orgId, projectId, sessionId)
  ...
  return (
    <div>
      <div className="flex">
        <div className="flex-[3] min-w-0 lg:mr-[290px] pr-8 border-r border-neutral-200">
            ...
            leftMeta={
              <div className="mb-10">
                <Link
                  ...
                  className="text-sm text-primary hover:underline mb-2 block cursor-pointer"
                >
                ...
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  >
                    {sessionService.SESSION_STATUS_LABEL[session.status]}
                  </Badge>
                  {progress && (
                    <Badge
                      variant="outline"
                ...
                {canSave && (
                  <>
                    ...
                    {canLock && session?.status !== 'locked' && (
                      <Button
                        variant="primary"
                        className="bg-neutral-900 gap-1.5"
                        onClick={handleLock}
                        loading={lockLoading}
                      >
                        <Lock size={16} />
          ...
          {canReopen && (
            <div className="flex gap-2 mb-6">
              <Button variant="outline" tone="success" size="sm" onClick={handleReopen} loading={reopenLoading}>
                Reopen session
              </Button>
            </div>
          )}
          ...
          {isLocked && (
            <div className="flex items-center gap-2 border border-warning/50 bg-warning/10 p-4 mb-6">
          ...
          <div className="space-y-8">
            {orderedSectionEntries.map(([section, questions]) => (
              <div key={section} id={`section-${section}`} className="scroll-mt-24">
                <Typography as="h2" size="lg" weight="semibold" className="mb-4 capitalize text-neutral-400">
                  {section}
                </Typography>
                    ...
                    return (
                      <div key={q.id} id={order != null ? `question-order-${order}` : undefined}>
                        <QuestionItem
                          ...
                          onAiImprove={canSave ? (question) => setAiImproveQuestion(question) : undefined}
                          questionOrder={order}
        ...
        {(progress || orderedSectionEntries.length > 0) && (
          <div className="hidden lg:block fixed right-0 top-[64px] h-[calc(100%-64px)] w-[320px]">
            <div className="h-[100%] overflow-hidden flex flex-col relative">
              {/* Decorative corner pattern (Figma) */}
              <div className="absolute bottom-0 right-0 pointer-events-none flex flex-col gap-0.5">
                <Image src="/illustrations/corner_pattern.svg" alt="Corner pattern" width={150} height={150} />
              </div>

              <div className="flex-1 overflow-auto flex flex-col p-6 relative z-10">
                  ...
                  <>
                    ...
                    <div className="space-y-4">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="text-neutral-900">{progress.required_answered} / {progress.required_total}</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-neutral-600">Optional answered</span>
                        <span className="text-neutral-900">{progress.optional_answered} / {progress.optional_total}</span>
                      </div>
                ...
                {rightPanelTab === 'outline' && (
                  <div className="space-y-0">
                    <Typography variant="small" weight="medium" className="text-neutral-500 mb-3 block">
                      Jump to section
                    </Typography>
                    {orderedSectionEntries.map(([section]) => (
                      <button
                        ...
                        className="w-full px-0 py-2.5 text-left text-sm text-neutral-900 hover:bg-neutral-100 capitalize"
                      >
                        {section}
                      </button>
                    ))}
                ...
                {FEATURES.aiClarityAssessment && rightPanelTab === 'clarity' && (
                  <ClarityPanel
                    ...
                    missingCount={claritySummary?.missing_assessments ?? claritySummary?.stats?.missing_assessments ?? 0}
              ...
              <div className="p-6 shrink-0 relative z-10 flex gap-1 flex-wrap">
                {(['progress', 'outline', 'evidence', ...(FEATURES.aiClarityAssessment ? ['clarity'] : [])] as const).map((tab) => (
                  <button
                    key={tab}
                    type="button"
                    onClick={() => setRightPanelTab(tab as 'progress' | 'outline' | 'clarity' | 'evidence')}
                    className={cn(
                      'py-2.5 px-4 text-sm transition-opacity',
                      rightPanelTab === tab
      ...
      {FEATURES.aiClarityAssessment && clarityModalOrder != null && (
        <ClarityDetailsModal
          ...
          questionPrompt={orderedQuestions.find((q) => questionOrderMap[q.id] === clarityModalOrder)?.prompt}
          onReAssess={
            canAssessClarity
              ? () => {
                  const q = orderedQuestions.find((q) => questionOrderMap[q.id] === clarityModalOrder)
