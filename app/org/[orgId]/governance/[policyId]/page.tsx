...
import type { GovernanceRule } from '@/types/governance'
import { GOVERNANCE_ACTION_KEYS, GOVERNANCE_EFFECTS } from '@/constants/governance.constants'
import { ApiError } from '@/types/api'
import { GovernanceConditionBuilder } from '@/shared/components/governance/GovernanceConditionBuilder'
import { GovernanceConditionJsonEditor } from '@/shared/components/governance/GovernanceConditionJsonEditor'
import {
  createEmptyConditionGroup,
  formatConditionsJson,
  parseConditionsJson,
  summarizeConditionGroup,
} from '@/utils/governanceConditions'
...
export default function GovernancePolicyDetailPage() {
  ...
  const [conditionsJson, setConditionsJson] = useState(formatConditionsJson(createEmptyConditionGroup('all')))
  ...
  const handleArchivePolicy = async () => {
    try {
      await governanceService.archiveGovernancePolicy(orgId, policyId)
  ...
  const handleCreateRule = async () => {
    ...
    setSaving(true)
    try {
      await governanceService.createGovernanceRule(orgId, policyId, {
  ...
  const handleArchiveRule = async (ruleId: string) => {
    try {
      await governanceService.archiveGovernanceRule(orgId, ruleId)
    ...
    <Box className="space-y-6">
      ...
      {canManage && policy.status !== 'archived' ? (
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" tone="error" onClick={() => void handleArchivePolicy()}>
            Archive policy
          </Button>
        </div>
      ) : null}
      ...
      {showRuleForm && canManage ? (
        <div className="space-y-3 rounded-md border border-border p-4">
          ...
          <Button variant="primary" size="sm" loading={saving} onClick={() => void handleCreateRule()}>
            Create rule
          </Button>
        </div>
      ) : null}
          ...
          Object.entries(rulesByAction).map(([action, actionRules]) => (
            <div key={action} className="space-y-2">
              ...
              {actionRules.map((rule) => (
                <div key={rule.id} className="rounded-md border border-border p-4 space-y-2">
                  ...
                  <Typography variant="small">{summarizeConditionGroup(rule.conditions)}</Typography>
