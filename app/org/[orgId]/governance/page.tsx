...
import type { GovernancePolicyListItem, GovernancePreset, GovernanceStatusResult } from '@/types/governance'
import { ApiError } from '@/types/api'
import { GovernanceStatusBanner } from '@/shared/components/governance/GovernanceStatusBanner'
import { GovernanceSimulator } from '@/shared/components/governance/GovernanceSimulator'
import { PresetPreviewModal } from '@/shared/components/governance/PresetPreviewModal'
...
export default function GovernancePoliciesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [policiesRes, statusRes] = await Promise.all([
        governanceService.listGovernancePolicies(orgId, {
          status: statusFilter || undefined,
          scope_type: scopeFilter || undefined,
          search: search.trim() || undefined,
          include_archived: statusFilter === 'archived' ? true : undefined,
        }),
        governanceService.getGovernanceStatus(orgId).catch(() => null),
      ])
      setItems(policiesRes.items)
      setTotal(policiesRes.total)
      setStatusInfo(statusRes)
      const presetRes = await governanceService.listGovernancePresets(orgId).catch(() => ({ items: [] }))
  ...
  const handleCreate = async () => {
    ...
    setCreating(true)
    try {
      await governanceService.createGovernancePolicy(orgId, {
  ...
  const handleApplyPreset = async (presetKey: string) => {
    ...
    try {
      await governanceService.applyGovernancePreset(orgId, {
  ...
  return (
    <Box className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <Typography variant="small" tone="muted">
            Configure org governance rules. Policies are inactive by default until activated.
            Active policies are enforced on document export, deliverables, templates, and workflow transitions.
          </Typography>
        </div>
        {canManage ? (
      ...
      <div className="grid gap-3 md:grid-cols-3">
        <Input label="Search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Name or policy key" />
        <Select
          label="Status"
          value={statusFilter}
      ...
      {showCreate && canManage ? (
        <div className="space-y-3 rounded-md border border-border p-4">
          <Input label="Policy key" value={policyKey} onChange={(e) => setPolicyKey(e.target.value)} placeholder="my_policy_key" />
          <Input label="Name" value={policyName} onChange={(e) => setPolicyName(e.target.value)} placeholder="My policy" />
              ...
              <li key={item.id} className="rounded-md border border-border p-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="space-y-1">
                    <Link href={ROUTES.org.governancePolicy(orgId, item.id)} className="font-medium text-primary hover:underline">
                      {item.name}
                    </Link>
                    <Typography variant="small" tone="muted">
                    ...
                    <Typography variant="small" tone="muted">
                      Priority {item.priority} · {item.rule_count} rules ({item.active_rule_count} active) · Updated{' '}
                      {new Date(item.updated_at).toLocaleDateString()}
                    </Typography>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge variant="soft" tone={item.status === 'active' ? 'success' : 'neutral'} size="sm">
                      {item.status}
                    </Badge>
                    <Link href={ROUTES.org.governancePolicy(orgId, item.id)} className="text-sm text-primary hover:underline">
                      View
                    </Link>
                  </div>
                </div>
      ...
      {presets.length > 0 && canManage ? (
        <div className="space-y-2">
          ...
          <ul className="space-y-2">
            {presets.map((preset) => (
              <li key={preset.preset_key} className="flex flex-wrap items-center justify-between gap-2 rounded border border-border p-3">
                <div>
                  <Typography variant="small" className="font-medium">
                    {preset.name}
                  </Typography>
                ...
                <Button variant="outline" size="sm" onClick={() => setPresetPreviewKey(preset.preset_key)}>
                  Preview & apply
                </Button>
              </li>
            ))}
      ...
      {canEvaluate ? (
        <GovernanceSimulator orgId={orgId} canViewRuleDetails={canViewRules} />
      ) : null}
