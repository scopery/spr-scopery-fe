...
export async function listAgentControlPresets(
  ...
  const res = await apiClient.get<AgentControlPreset[]>(AGENT_CONTROL_ENDPOINTS.presets(orgId))
...
export async function listOrgAgents(
  ...
  const res = await apiClient.get<OrgAgentListItem[] | { items?: OrgAgentListItem[]; total?: number }>(
    AGENT_CONTROL_ENDPOINTS.agents(orgId, params)
  )
...
export async function listOrgModelPolicies(
  ...
  const res = await apiClient.get<
    OrgModelPolicyListItem[] | { items?: OrgModelPolicyListItem[]; total?: number }
  >(AGENT_CONTROL_ENDPOINTS.modelPolicies(orgId, params))
  const { items } = normalizeItemList(res)
  return {
    items,
    total: !Array.isArray(res) && typeof res?.total === 'number' ? res.total : items.length,
...
export async function listPromptTemplates(
  ...
  params?: {
    ...
    const res = await apiClient.get<
    PromptTemplateListItem[] | { items?: PromptTemplateListItem[]; total?: number }
  >(AGENT_CONTROL_ENDPOINTS.prompts(orgId, params))
  const { items } = normalizeItemList(res)
  return {
    items,
    total: !Array.isArray(res) && typeof res?.total === 'number' ? res.total : items.length,
...
export async function listTemplatePromptBindings(
  ...
  const res = await apiClient.get<
    TemplatePromptBinding[] | { items?: TemplatePromptBinding[]; total?: number }
  >(AGENT_CONTROL_ENDPOINTS.templatePromptBindings(orgId, params))
  const { items } = normalizeItemList(res)
  return {
    items,
    total: !Array.isArray(res) && typeof res?.total === 'number' ? res.total : items.length,
...
export async function listOrgAgentRuns(
  ...
  const res = await apiClient.get<OrgAgentRun[] | { items?: OrgAgentRun[]; total?: number }>(
    AGENT_CONTROL_ENDPOINTS.runtimeRuns(orgId, params)
  )
  const { items } = normalizeItemList(res)
  return {
    items,
