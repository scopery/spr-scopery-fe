...
import { Archive, ArchiveRestore, ChevronDown, ChevronUp, CircleArrowOutUpLeft, Copy, FileJson, Pencil, Plus, Save, Upload } from 'lucide-react'
import { Typography, Button, Badge, Input, Textarea, Select, Checkbox, Modal, PageSkeleton } from '@/shared/ui'
import { ROUTES } from '@/constants/routes'
import * as adminTemplatesApi from '../../infrastructure/api/admin-templates.api'
...
import type { SystemQuestion } from '../../domain/model/template'
...
import {
  ...
  SECTION_OPTIONS,
  Q_TYPE_OPTIONS,
} from '../../domain/rules/template.rules'
...
export function AdminTemplateDetailView() {
  ...
  const [editQuestion, setEditQuestion] = useState<SystemQuestion | null>(null)
  const [archiveConfirm, setArchiveConfirm] = useState<SystemQuestion | null>(
    null
  )
  ...
  const handleArchive = async (q: SystemQuestion) => {
  ...
  const handleUnarchive = async (q: SystemQuestion) => {
    setActionLoading(true)
    try {
    ...
    <div className="mx-auto p-6">
      ...
      {isDraft && activeCount < 1 && (
        <Typography
          as="div"
          variant="small"
          className="bg-warning/10 border-warning/30 mb-4 border p-3 text-neutral-700"
        >
          ...
          {isDraft ? (
            <>
              ...
              <Button
                ...
                loading={overviewSaving} icon={<Save size={16} />}>
                Save changes
              </Button>
            </>
          ) : (
        ...
        <div>
          {isDraft && (
            <div className="mb-4 flex flex-wrap gap-2">
              ...
              <Button
                ...
                className="gap-1" icon={<Plus size={16} />}>
                Add by JSON
              </Button>
            </div>
          )}
          <div className="overflow-hidden bg-white">
            <table className="w-full text-left">
              <thead className="border-b border-neutral-500">
                <tr>
                  ...
                  {isDraft && (
                    <th className="min-w-[14rem] whitespace-nowrap px-4 py-2 text-sm font-normal text-neutral-700">Actions</th>
                  )}
                </tr>
              </thead>
              <tbody>
                  ...
                  questions.map((q) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2">
                        <Badge variant="solid" tone={q.status === 'active' ? 'success' : 'neutral'}>
                          {q.status === 'active' ? 'Active' : q.status === 'archived' ? 'Archived' : q.status}
                        </Badge>
                      </td>
            ...
            {template.questions.length === 0 && (
              <Typography as="div" variant="small" tone="muted" className="px-4 py-8 text-center">
          ...
          <div className="w-full max-w-sm bg-white p-6 shadow-xl">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button
                ...
                loading={actionLoading} icon={<Archive size={16} />}>
                Archive
              </Button>
            </div>
          </div>
