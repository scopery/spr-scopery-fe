...
import type { AIFeedbackStatus } from '../../domain/model/ai-run-feedback'
import { FEEDBACK_STATUSES } from '../../domain/model/ai-run-feedback'
...
export function AIAgentQualityPanel({ agentId, orgId }: AIAgentQualityPanelProps) {
  ...
  if (panel.loading || !panel.summary) return <Skeleton variant="rectangular" width="100%" height={120} />
              ...
              {panel.feedback.map((item) => (
                <tr key={item.feedbackId} className="border-b border-neutral-100">
                  ...
                  <td className="px-4 py-3">
                    <Button variant="outline" onClick={() => panel.setSelected(item)} icon={<Eye size={16} />}>
                      Review
                    </Button>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-lg rounded-lg border border-neutral-200 bg-white p-6 shadow-lg">
            ...
            <div className="space-y-2">
              <Typography variant="small">Run ID: {panel.selected.runId}</Typography>
              <Typography variant="small">Rating: {panel.selected.rating}</Typography>
              <Typography variant="small">
                Category: {panel.selected.feedbackCategory ?? '—'}
              </Typography>
              <Typography variant="small">
                Comment: {panel.selected.feedbackTextPreview ?? '—'}
              </Typography>
              <Typography variant="small">
                Run status: {panel.selected.runStatus ?? '—'}
              </Typography>
            </div>
              ...
              {(['reviewed', 'action_required', 'resolved', 'dismissed'] as AIFeedbackStatus[]).map(
                (status) => (
                  <Button
                    ...
                    onClick={() => void panel.updateStatus(status)} icon={<Check size={16} />}>
