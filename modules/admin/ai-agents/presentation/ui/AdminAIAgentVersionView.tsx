...
import { Typography, Button, Textarea, Select, ConfirmDialog, PageSkeleton } from '@/shared/ui'
...
import { AIAgentVersionStatusBadge, formatEstimatedCost } from './ai-agent-badges'
...
export function AdminAIAgentVersionView() {
  ...
  } = useAiAgentVersion(agentId, versionId)
  ...
  const isEditable = version?.status === 'draft' || version?.status === 'testing'
  ...
  useEffect(() => {
    ...
    setParametersJson(version.parametersJson ? JSON.stringify(version.parametersJson, null, 2) : '')
    setOutputSchemaJson(
      version.outputSchemaJson ? JSON.stringify(version.outputSchemaJson, null, 2) : ''
    )
  }, [version])
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await aiAgentsApi.updateAgentVersion(agentId, versionId, {
  ...
  const handlePublish = async () => {
    setPublishing(true)
    try {
      await aiAgentsApi.publishAgentVersion(agentId, versionId)
    ...
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          {isEditable ? (
            <>
              <Button variant="outline" onClick={handleSave} disabled={saving} icon={<Save size={16} />}>
                Save draft
              </Button>
              <Button
                variant="primary"
                className="bg-neutral-900"
                onClick={() => setPublishOpen(true)} icon={<Upload size={16} />}>
                Publish
              </Button>
            </>
          ) : null}
        ...
        <div>
          ...
          {selectedModel ? (
            <div className="mt-3 rounded-lg border border-neutral-200 bg-neutral-50 p-4 text-sm">
              ...
              <Typography variant="small" size="xs" tone="muted" className="mt-1">
              ...
              {!selectedModel.active ? (
                <Typography variant="small" size="xs" className="mt-2 text-amber-700">
              ...
              {!selectedModel.pricing ? (
                ...
                <Typography variant="small" size="xs" tone="muted" className="mt-2">
                  Pricing:{' '}
                  {formatEstimatedCost(
