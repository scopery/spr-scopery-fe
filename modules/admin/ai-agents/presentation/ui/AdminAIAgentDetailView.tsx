...
import { ArrowLeft, ArrowRight, Eye, Pencil, Plus, Upload } from 'lucide-react'
import { Typography, Button, Select, ConfirmDialog, PageSkeleton } from '@/shared/ui'
import { ROUTES } from '@/constants/routes'
import { FEATURES } from '@/config/features'
import { useAuth } from '@/modules/auth/auth/context/AuthContext'
import * as aiAgentsApi from '../../infrastructure/api/ai-agents.api'
...
import {
  ...
  formatEstimatedCost,
  formatTokens,
} from './ai-agent-badges'
import { AIAgentUsageSummaryCards } from './ai-agent-usage-summary-cards'
import { AIAgentRoutingPanel, AIAgentPlaygroundPanel, AIAgentQualityPanel } from '@/modules/admin'
import { useAiAgentDetail } from '../hooks/useAiAgents'
import type { AIRunLogItem, AIUsageSummary } from '../../domain/model/ai-agent-control'
...
export function AdminAIAgentDetailView() {
  ...
  const loadUsage = useCallback(async () => {
    ...
    try {
      const data = await aiAgentsApi.getAgentUsageSummary(agentId, { org_id: orgId })
      setUsage(data)
    } catch (err) {
  ...
  const loadRunLogs = useCallback(async () => {
    if (!agentId || !orgId) return
    try {
      const data = await aiAgentsApi.listAgentRunLogs(agentId, {
        org_id: orgId,
  ...
  const handleCreateDraft = async () => {
    ...
    try {
      const version = await aiAgentsApi.createDraftFromPublished(agentId)
  ...
  const handlePublish = async () => {
    ...
    setPublishing(true)
    try {
      await aiAgentsApi.publishAgentVersion(agentId, publishVersionId)
      ...
      <div className="mb-6">
        ...
        {!agent.publishedVersion ? (
          <Typography variant="small" tone="muted" className="mt-2 text-amber-700">
            No published version. Future AI runs will not use this agent until a version is
            published.
          </Typography>
        ) : null}
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            <div className="rounded-lg border border-neutral-200 p-4">
              <Typography variant="xs" className="mb-2 uppercase text-neutral-500">
                Published version
              </Typography>
              <Typography>
                {publishedModel
            ...
            <div className="rounded-lg border border-neutral-200 p-4">
              ...
              <Typography>{agent.usageThisMonth?.totalRuns?.toLocaleString() ?? '—'}</Typography>
            </div>
            <div className="rounded-lg border border-neutral-200 p-4">
              <Typography variant="xs" className="mb-2 uppercase text-neutral-500">
                Est. cost this month
              ...
              {agent.versions.map((version) => {
                ...
                return (
                  <tr key={version.id} className="border-b border-neutral-100">
                    ...
                    <td className="px-4 py-3">{new Date(version.updatedAt).toLocaleString()}</td>
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        {editable ? (
                          <Button
                            variant="primary"
                            onClick={() => setPublishVersionId(version.id)} icon={<Upload size={16} />}>
                            Publish
                          </Button>
                        ) : null}
                      </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    onClick={() => setRunLogOffset(Math.max(0, runLogOffset - 20))} icon={<ArrowLeft size={16} />}>
                    Previous
                  </Button>
                  <Button
                    variant="outline"
                    disabled={runLogOffset + 20 >= runLogsTotal}
                    onClick={() => setRunLogOffset(runLogOffset + 20)} icon={<ArrowRight size={16} />}>
                    Next
                  </Button>
                </div>
              </div>
