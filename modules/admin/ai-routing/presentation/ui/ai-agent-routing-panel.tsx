...
import { Check, Play, Plus, Save } from 'lucide-react'

import { Typography, Button, Input, Select, ConfirmDialog, Skeleton } from '@/shared/ui'
...
export function AIAgentRoutingPanel({ agentId, agentKey, orgId }: AIAgentRoutingPanelProps) {
  ...
  if (panel.loading) return <Skeleton variant="rectangular" width="100%" height={120} />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-2">
        <Button onClick={() => panel.setFormOpen(true)} icon={<Plus size={16} />}>
          Create routing rule
        </Button>
        <Button variant="outline" onClick={() => void panel.runDryRun()} disabled={panel.dryRunning} icon={<Play size={16} />}>
      ...
      {panel.dryRunResult ? (
        <div className="rounded-lg border border-neutral-200 bg-neutral-50 p-4">
          ...
          <Typography variant="small">Default: {panel.dryRunResult.defaultModelName}</Typography>
          <Typography variant="small">Selected: {panel.dryRunResult.selectedModelName}</Typography>
          <Typography variant="small">
            Routing applied: {panel.dryRunResult.routingApplied ? 'Yes' : 'No'}
          </Typography>
          <Typography variant="small">{panel.dryRunResult.routingReason}</Typography>
        </div>
      ) : null}
            ...
            <tbody>
              {panel.rules.map((rule) => (
                <tr key={rule.ruleId} className="border-b border-neutral-100">
                  ...
                  <td className="px-4 py-3">
                    <Button variant="outline" onClick={() => void panel.toggleActive(rule)} icon={<Check size={16} />}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-lg border border-neutral-200 bg-white p-6 shadow-lg">
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button onClick={() => void panel.handleCreate()} icon={<Save size={16} />}>Save</Button>
            </div>
          </div>
        </div>
      ) : null}
