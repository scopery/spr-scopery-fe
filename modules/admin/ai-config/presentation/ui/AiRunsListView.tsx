...
import type { AiPurpose, AiRunStatus } from '../../domain/model/ai'
import { useRouter } from 'next/navigation'
import { FEATURES } from '@/config/features'
import { useAdminAiRuns } from '../hooks/useAdminAi'

export function AiRunsListView() {
  ...
  return (
    <div className="container mx-auto p-8">
      <div className="mb-6 flex items-center justify-between">
        ...
        <Button variant="outline" onClick={() => router.push('/admin/ai')} icon={<ArrowLeft size={16} />}>
          Back to Configs
        </Button>
      </div>
      ...
      <div className="mb-6 flex gap-4 rounded-lg bg-neutral-50 p-4">
        ...
        <div className="flex items-end">
          <Button variant="outline" onClick={() => loadRuns()} icon={<RefreshCw size={16} />}>
            Refresh
          </Button>
        </div>
      </div>
                ...
                {runs.map((run) => (
                  <tr key={run.id} className="hover:bg-neutral-50">
                    ...
                    <td className="px-4 py-3">
                      <Button
                        variant="ghost"
                        onClick={() => router.push(`/admin/ai/runs/${run.id}`)} icon={<Eye size={16} />}>
                        View
                      </Button>
                    </td>
                  </tr>
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={page.offset === 0} icon={<ArrowLeft size={16} />}>
                Previous
              </Button>
              <Button
                variant="outline"
                onClick={handleNextPage}
                disabled={page.offset + page.limit >= page.total} icon={<ArrowRight size={16} />}>
                Next
              </Button>
            </div>
          </div>
