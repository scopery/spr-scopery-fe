...
import type { SessionListItem } from '@/modules/sessions/session/model/session'
...
} from '@/modules/ai-document-intelligence/document-ai'
...
export function useProjectAIActionsMenu({
  ...
  const savePreview = async () => {
    ...
    try {
      ...
      setPreviewOpen(false)
      router.push(ROUTES.workspace.document(orgId, res.document.id, projectId))
    } catch (err) {
      handleError(err)
    } finally {
      setLoading(false)
