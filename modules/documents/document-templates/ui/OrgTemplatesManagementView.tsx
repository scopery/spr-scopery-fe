...
import { documentTemplatesApi } from '@/modules/documents'
import type { DocumentTemplate, TemplateStatus } from '@/modules/documents'
import { DOCUMENT_TYPE_OPTIONS } from '@/modules/documents'
import {
  canArchiveTemplate,
...
export function OrgTemplatesManagementView() {
  ...
  const handleArchive = async () => {
    ...
    setActionLoading(true)
    try {
      await documentTemplatesApi.archiveTemplate(orgId, archiveTarget.id)
  ...
  return (
    <Box className="mx-auto max-w-5xl">
      <Stack direction="row" justify="between" align="start" className="mb-6 flex-wrap gap-4">
        <div>
          <Typography as="h1" size="2xl" weight="semibold">
            Templates
          </Typography>
          <Typography variant="small" tone="muted" className="mt-1 max-w-2xl">
            Create reusable document structures for project notes, meetings, decisions, research,
            and summaries.
          </Typography>
        </div>
        <Button
