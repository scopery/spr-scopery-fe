...
import { DeliverableHistoryPanel } from '@/modules/documents'
import * as deliverablesApi from '../api/deliverables.api'
import type { DeliverableHistoryItem } from '@/modules/documents/deliverables'
...
const mockList = vi.mocked(deliverablesApi.listDeliverableHistory)
...
describe('DeliverableHistoryPanel', () => {
  ...
  it('shows stale indicator when readiness is outdated', async () => {
    mockList.mockResolvedValue({
      items: [
        makeItem({ readiness_stale: true, readiness_status: 'needs_review', warning_count: 1 }),
      ],
      total: 1,
    })
