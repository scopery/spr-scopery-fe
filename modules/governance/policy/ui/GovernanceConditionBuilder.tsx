...
export function GovernanceConditionBuilder({
  ...
  const updateClauses = (next: typeof clauses) => {
    onChange(mode === 'all' ? { all: next } : { any: next })
  }

  const updateClause = (index: number, patch: Partial<(typeof clauses)[number]>) => {
    const next = clauses.map((clause, i) => (i === index ? { ...clause, ...patch } : clause))
    updateClauses(next)
  }

  const addClause = () => {
    updateClauses([...clauses, { field: 'workflow_status', operator: 'equals', value: 'draft' }])
  }
