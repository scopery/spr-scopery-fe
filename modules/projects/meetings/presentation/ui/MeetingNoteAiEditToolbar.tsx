...
import { useAiTextRewrite } from '@/modules/ai-assistant/presentation/hooks/useAiTextRewrite'
import { cn } from '@/utils/cn'

interface MeetingNoteAiEditToolbarProps {
...
/**
 * Floating AI Edit for meeting notes textarea — same flow as document AI Edit:
 * select text → instruct → review → accept replaces selection.
 */
export function MeetingNoteAiEditToolbar({
  textareaRef,
  value,
  workspaceId,
  onApply,
}: MeetingNoteAiEditToolbarProps) {
  const rewrite = useAiTextRewrite(workspaceId)
  ...
  const isOpen = hasSelection || rewrite.phase !== null
  ...
  return (
    <div
      ...
      role="toolbar"
      aria-label="Meeting note AI edit"
    >
      {rewrite.phase === null ? (
...
/** Approximate selection rect inside a textarea via mirror element. */
