...
> Spreadsheet Test Case Catalog, lifecycle/result separation, bulk editing, Test Steps, and
> Test Run Result requirements:
> [`TEST_CASE_CATALOG_AND_EXECUTION_BE_API_REQUIREMENTS.md`](./TEST_CASE_CATALOG_AND_EXECUTION_BE_API_REQUIREMENTS.md).
> The linked document records the 2026-07-30 live OpenAPI schema and remaining response/filter
> gaps after V176/V177/V178 deployment.
...
```json
{
  ...
  "sourceVerificationResultId": null
}
```

Optional source linkage (V188):

- `sourceTestCaseResultId` — functional run result; BE resolves `testRunId` + `testCaseId`, back-links `defectId` on the result
- `sourceVerificationResultId` — NFR verification result; BE resolves `testRunId` + `verificationCaseId`

**GET list / detail — `source` enrichment** (`DefectSourceResponse`, null when manual):
...
```json
{
  "source": {
    ...
    "testRunId": "<uuid>",
    "testRunName": "Sprint 12 regression",
    "resultId": "<uuid>",
    "resultStatus": "FAILED",
    "caseKind": "FUNCTIONAL",
    "caseId": "<uuid>",
    ...
    "resultComment": "Firefox returns 500"
  }
}
```

`kind`: `FUNCTIONAL_RESULT` | `VERIFICATION_RESULT`  
Lifecycle action responses may return `source: null` — FE should rely on GET list/detail for provenance.

`severity` values: `BLOCKER` | `CRITICAL` | `MAJOR` | `MINOR` | `TRIVIAL` (confirm live BE)  
`status` flow: `OPEN` / `TRIAGED` → `IN_PROGRESS` → `FIXED` → `READY_FOR_RETEST` → `CLOSED` (plus reopen/reject)
...
```json
{
  ...
  "results": [
    {
      "suggestionItemId": "<uuid>",
      "status": "APPLIED",
      "domainAction": "CREATE_TASK",
      "targetType": "TASK",
      "targetId": "<uuid>"
    }
  ]
