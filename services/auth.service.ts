/**
 * Auth service — v2 API.
 * - login/register return { user, session } directly (no ok/data envelope).
 * - Google: GET /api/v2/auth/google?redirectTo=... → { url } → window.location.href = url
 */
...
// BE returns { access_token, user, profile } (flat, not Supabase's { session: { access_token } })
...
export async function register(payload: RegisterPayload): Promise<AuthSession> {
  ...
  const res = await apiClient.post<BeAuthResponse>(url, {
    email: payload.email,
...
/**
 * Google OAuth — v2 API.
 * GET /api/v2/auth/google?redirectTo=<FE_CALLBACK_URL> → { url } → window.location.href = url
 */
