/**
 * Organization service — v2 API
 */

import { ORG_ENDPOINTS } from '@/constants/endpoints'
...
export async function listOrgs(params?: { limit?: number; offset?: number }): Promise<OrgListResponse> {
...
export async function createOrg(name: string): Promise<{ id: string; name: string; status: string; created_at: string }> {
  const url = ORG_ENDPOINTS.create()
  return apiClient.post<{ id: string; name: string; status: string; created_at: string }>(url, { name })
}
...
export async function getOrgMembers(orgId: string, params?: { limit?: number; offset?: number }): Promise<OrgMembersResponse> {
