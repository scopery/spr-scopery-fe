...
import { clearSessionStorage, getSessionFromCookie, setSessionStorage } from '@/shared/lib/apiClient'
import * as authService from '@/services/auth.service'
import * as profileService from '@/services/profile.service'
import * as orgService from '@/services/org.service'
import type { AuthSession, LoginPayload, RegisterPayload, Profile } from '@/types/auth'
import type { OrgListItem } from '@/services/org.service'
import { ROUTES } from '@/constants/routes'
import { ApiError } from '@/types/api'

type BootstrapStatus = 'loading' | 'needs_login' | 'suspended' | 'needs_onboarding' | 'ready'
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const runBootstrap = useCallback(async () => {
    ...
    try {
      ...
      const orgList = await orgService.listOrgs({ limit: 20, offset: 0 })
      ...
      if (!validDefault) {
        await orgService.setDefaultOrg(chosenId)
      }
  ...
  const value = useMemo(
    ...
    [session, profile, orgs, currentOrgId, bootstrapStatus, login, register, logout, setCurrentOrgId, refreshBootstrap]
  )
